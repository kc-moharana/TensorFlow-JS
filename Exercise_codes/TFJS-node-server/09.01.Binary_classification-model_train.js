console.log('Design a Commandline User Interface to accept a house_space and approx the house price and predict likelihood of gettign waterfront');
console.log('===========');
...
async function load_data(){
  ...
  const labels_value = points.map(p=>p.class);

  const features_tensor = tf.tensor2d(features_value);
  const labels_tensor = tf.tensor2d(labels_value, [labels_value.length,1]);
...
async function run(){
  ...
  while (true) {
    ...
    if(isNaN(input_value.house_sqft%2) || isNaN(input_value.price%2) ){
      ...
      console.log(`\n#likelihood of getting a waterfront house : \$ ${(prediction_output_value*100).toFixed(1)}% `);
    });
    }
  }
}
